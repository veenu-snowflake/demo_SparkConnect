...
print("""
  ...
  ─────────────────────     ─────────────────────────────────
  RDDs                      Use DataFrame API (works on Connect)
  Structured Streaming      Snowpipe Streaming / Dynamic Tables
  MLlib                     Snowpark ML / Cortex ML Functions
  GraphX                    No direct equivalent
  Delta Lake format         Iceberg Tables on Snowflake
  Avro / ORC readers        Parquet or CSV (supported)
  SparkContext              Not needed — use SparkSession only
  Java / Scala UDFs         Python UDFs only (PySpark 3.5.x)

  KEY TAKEAWAY:
  "Change the connection string, not the code."
  If your workload is DataFrame + SQL + Python UDFs, it runs
  on Snowpark Connect with ZERO code changes beyond session setup.
""")
